...
// Funktion zum Anzeigen / Ausblenden der Elemnte des addFilmLayouts
...
//Unterfunktionen implementieren
//Funktion zum Aktivieren/Desaktivieren des addFilmButtons
...
function displayFilms(filteredFilms){
  ...
  //Filmlist - Quelle auswählen und sortieren
  ...
  films.forEach((film, index)=>{
   ...
   //Listenelement der Filmlisten hinzufügen
...
//Funktion suche von Filmen

function searchFilms(){
 const searchTerm = searchInput.value.trim().toLowerCase();//Falls sich der User vertippen sollte
 ...
 let filteredFilms;
  if(searchTerm === "favoriten"){
   ...
   filteredFilms = dummyFilmList.filter((film)=> {
    return(
     film.title.toLocaleLowerCase().includes(searchTerm) ||
      film.genre.toLocaleLowerCase().includes(searchTerm));
   });
  }
...
//Funktion zum Anzeigen/ Ausblenden der Komponenten der Filmsuche
