...
const filmTitleInput =
  document.getElementById("filmTitle");
const filmGenreInput =
  document.getElementById("filmGenre");
const confirmAddFilmButton =
  document.getElementById(
    "confirmAddFilmButton"
  );
...
addFilmButton.addEventListener(
  "click",
  toggleAddFilmElements
);
confirmAddFilmButton.addEventListener(
  "click",
  addFilm
);
...
function toggleAddFilmButton() {
  addFilmButton.classList.toggle(
    "disabled"
  );
}
...
function addFilm() {
  const title =
    filmTitleInput.value.trim();
  const genre =
    filmGenreInput.value.trim();
...
function displayFilms(filteredFilms) {
  ...
  // Filmeliste - Quelle auswählen + sortieren
  const films =
    filteredFilms ?? dummyFilmList;
  films.sort((a, b) =>
...
function searchFilms() {
  ...
  if (searchTerm === "favoriten") {
    filteredFilms =
      dummyFilmList.filter(
        (film) => film.favorite
      );
  } else {
    filteredFilms =
      dummyFilmList.filter((film) => {
        return (
          film.title
            .toLowerCase()
            .includes(searchTerm) ||
